...
这些 JSON 文件是已获授权的本地工件，不是浏览器操作授权。生成它们前，v2.0 必须先完成：启动方式 → 收藏/点赞范围 → 本机 OCR 检查与选择 → 当前回合的浏览器授权、登录页与 JavaScript 检查 → 每组条数（最大 `200`）和暂停分钟数设置。

列表读取完成并合并所有已保存分段后，才允许依据真实图文/视频分布决定视频内容分类。分类方案预览默认只显示专辑和数量；JSON 仍保留逐条数据，只有用户明确要求才向其展开条目。任何创建专辑或移动都需要独立的后续确认。

安全验证或页面状态异常时，保存已有工件并将状态写入 `xhs_safety_state.json`。用户处理后回复“继续整理”时，只检查当前授权页面，不重读已保存分段、不自动重试旧请求；恢复细节见 [失败恢复与续跑](recovery-and-resume.md)。
...
`visible_items.json` 来自收藏/点赞列表页。每个读取分段先独立保存，再在本机合并。只有用户一次确认自动继续、且当次任务执行器已验证支持受控续组时，任务才可按其已确认的每组条数（最大 `200`）和暂停分钟数请求下一组；旧的被动读取路径仍只代表当前一组。列表页的 `cover_image_url` / `image_urls` 和 `content_type` 都只是 observed 线索：图片必须保持 `image_urls_complete=false`、`image_count=null`，不能证明已经取得图文笔记的全部图片，也不能直接作为“全图 OCR 已完成”的依据。图文 OCR 开启时，必须先生成下面的 `image_items.json`。
...
首次完整整理完成后，`weekly_processed_note_ids.json` 记录已经完整进入过分类方案的 note id。它不包含浏览器登录信息，也不授予移动权限：

```json
{
  "schema_version": 1,
  "processed_note_ids": ["66d19b54000000001d03a93d", "694d3390000000002203ae33"]
}
```

`scripts/weekly_delta.py visible_items.json weekly_delta.json --baseline weekly_processed_note_ids.json` 默认只生成新增条目预览。只有在 `--commit-after-plan classification.json` 且该分类方案完整覆盖全部新增 id 后，才更新基线；缺少、重复或混入旧 id 的分类方案必须拒绝提交。
...
```json
{
  ...
  "new_note_count": 1,
  "new_note_ids": ["new-note-id"],
  "new_items": [{"id": "new-note-id", "title": "本周新增"}],
  "automatic_move": false
}
```

`weekly_delta.json` 只用于生成新内容的分类方案；它不创建专辑、不移动笔记，也不会更新基线，除非调用方显式给出完整分类方案的 `--commit-after-plan`。
...
- `ocr_run_fingerprint` 绑定 OCR pipeline 版本和实际 provider；Tesseract 路径还绑定 `--tesseract-lang`，Swift 路径还绑定 `ocr_image.swift` 内容 SHA256 所代表的脚本版本。任一项改变都必须重跑。Tesseract 默认且至少使用 `chi_sim+eng`；缺少其中任一语言包时不得运行单语 OCR。

### `video_transcripts.json`

只在所有列表分段已合并、真实视频数量已展示、且用户明确开启“根据视频实际内容分类”时生成。快速启动应先给出基于真实分布的推荐；完整启动可逐项选择声音、画面和 provider。脚本只处理明确识别为 `video` 的条目；成功项必须通过覆盖率校验，失败项必须保留 `status`、`stage`、`reason_code` 和 `error`。
...
向用户展示时先聚合成“分类方案预览”：拟用专辑、每个专辑数量、待人工复核数和失败数。默认不显示下方 JSON 中的逐条标题；用户确认需要审阅时，才展开指定专辑或全部条目。该预览和 JSON 落盘都不是执行授权。
...
每周任务生成的分类方案沿用同一契约，但输入范围只能是上次成功运行后新出现的内容。每周任务不得自动生成 `created_boards.json` 中的创建动作，也不得产生 `run_report.json` 的 `mode=execute`；只有用户当次确认浏览器后，才可从分类方案进入 dry-run 或执行流程。
