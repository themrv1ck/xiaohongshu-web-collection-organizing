...
description: "整理用户本人已登录的小红书收藏或点赞内容：先只读采集、分类并展示 dry-run，只有用户明确确认方案后才移动收藏到专辑。不得发布笔记、评论、点赞或取消点赞、关注、私信、清空收藏或自动运营账号。WorkBuddy 使用公开 GitHub Plugin 和独立可见浏览器；其他宿主须在用户同意后安装同一公开仓库的完整 Skill。"
---
...
读取专辑、创建专辑和归档笔记只允许使用小红书正式页面可见卡片、表单和“加入专辑”控件。必须按抓取时的 note id 回到来源列表点击真实卡片，禁止按标题搜索。禁止访问网页内部模块、注册自定义 chunk 或调用私有接口；出现 300031、安全验证、登录页或页面绑定变化立即停止。保护依据不是“已经收藏”或“进入任意专辑”，而是本 Skill 完成 execute 和最终完整回读后生成的 `xhs-skill-archive-registry-v2`；只有登记专辑的实时成员受保护。
...
- 不清空收藏，不操作 Skill 已归档保护的笔记。待归档的已有收藏只有明确同意后才可取消一次再重新收藏，并立即使用“加入专辑”；会改变排序，中途失败可能留下未收藏或未归入状态，必须提前说明，失败立即停止。
...
用户选“其他”时，不打开浏览器。先让用户准确复述内容现在在哪里、处理范围、期望结果、允许的读取/写入操作和本轮浏览器，再判断能否严格映射为本 Skill 的“收藏/点赞 → 分类 → dry-run → 明确确认 → 可见加入专辑 → 完整回读”流程。可实现才继续；不可实现要说明具体原因，并推荐用户先手工完成缺失步骤或改用可支持的范围。不得猜测或静默改成收藏。
...
向用户列明待创建专辑及隐私、每条收藏的当前位置与目标专辑、移动数量上限，以及 `visible_assignment_contract.risk_notice` 中取消后重收藏的动作和风险。此时不得写账号。

### 5. 明确确认后写入

只有用户确认上述完整方案后，才把原 `approval_digest` 传给 `xhs_workbuddy_execute`。不得修改计划、扩大数量或绕过工具直接执行脚本。新收藏须立即使用收藏后出现的“加入专辑”；已有收藏仅按批准的取消一次、重收一次流程进行，不能靠悬停。旧审批未包含新合同必须重新 prepare 并确认。
