...
REQUIRED_TESSERACT_LANGUAGES = ('chi_sim', 'eng')
DEFAULT_TESSERACT_LANG = '+'.join(REQUIRED_TESSERACT_LANGUAGES)
OCR_PIPELINE_VERSION = 'multi-image-ocr-v2-bilingual'
    ...
    try:
        proc = subprocess.run(
            [swift_bin, '-e', (
                ...
                'print(supported.contains("zh-Hans") && supported.contains("en-US") ? "ok" : "missing")'
            )],
            capture_output=True,
            text=True,
            timeout=15,
...
def tesseract_languages_ready(languages=REQUIRED_TESSERACT_LANGUAGES):
    return all(tesseract_language_ready(language) for language in languages)


def run_tesseract_ocr(image_path: Path, languages: str = DEFAULT_TESSERACT_LANG):
    tesseract = shutil.which('tesseract')
    if not tesseract:
        raise RuntimeError('tesseract not found')
    requested = [language for language in languages.split('+') if language]
    omitted = [language for language in REQUIRED_TESSERACT_LANGUAGES if language not in requested]
    if omitted:
        raise RuntimeError(
            'tesseract OCR 必须同时启用中文简体 chi_sim 和英文 eng；'
            f'当前缺少：{", ".join(omitted)}'
        )
...
def perform_ocr_for_items(items, output_path: Path, cache_dir: Optional[Path] = None, swift_script: Optional[Path] = None, timeout_sec: int = 20, force: bool = False, provider: str = 'auto', tesseract_lang: str = DEFAULT_TESSERACT_LANG):
