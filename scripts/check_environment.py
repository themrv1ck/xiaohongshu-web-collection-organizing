...
from xhs_ocr_common import (
    ...
    swift_vision_ready,
    tesseract_language_ready,
    tesseract_languages_ready,
)
...
def tesseract_ocr_probe():
    """Return readiness for the mandatory Simplified-Chinese plus English baseline."""
    found = check('tesseract')
    chi_sim = tesseract_language_ready('chi_sim') if found else False
    eng = tesseract_language_ready('eng') if found else False
    return {
        'found': found,
        'chi_sim': chi_sim,
        'eng': eng,
        'bilingual': bool(found and tesseract_languages_ready()),
    }
...
def main():
    ...
    parser.add_argument('--ocr', action='store_true', help='用户开启图文 OCR 后，检测本机中英文 OCR 能力')
    ...
    if args.capability_preflight:
        tesseract_probe = tesseract_ocr_probe()
        tesseract_found = tesseract_probe['found']
        tesseract_chi_sim = tesseract_probe['chi_sim']
        tesseract_eng = tesseract_probe['eng']
        tesseract_bilingual = tesseract_probe['bilingual']
        ...
        elif tesseract_found and tesseract_bilingual:
            ocr_provider = f'tesseract-{DEFAULT_TESSERACT_LANG}'
        else:
            ocr_provider = 'none'
        ...
        report = {
            ...
            'capabilities': {
                'ocr': {
                    ...
                    'source': 'local_probe',
                    'languages_required': ['zh-Hans', 'en-US'],
                    'tesseract_chi_sim': tesseract_chi_sim,
                    'tesseract_eng': tesseract_eng,
                    'tesseract_bilingual': tesseract_bilingual,
                    'missing': [] if ocr_ready else ['bilingual-ocr'],
            ...
            'next_step': '先选择快速启动或完整启动，再确认整理范围并询问是否开启中英文 OCR；完成本地列表统计后，才决定是否需要视频内容能力。',
        }
    ...
    tesseract_probe = tesseract_ocr_probe() if ocr_checked else {
        'found': False, 'chi_sim': False, 'eng': False, 'bilingual': False,
    }
    tesseract_found = tesseract_probe['found']
    tesseract_chi_sim = tesseract_probe['chi_sim']
    tesseract_eng = tesseract_probe['eng']
    ...
    checks = {
        ...
        'tesseract_eng': tesseract_eng,
        'tesseract_bilingual': tesseract_bilingual,
        'ocr_languages_required': ['zh-Hans', 'en-US'],
    ...
    checks['ocr_ready'] = bool(ocr_checked and (
        ...
        or (checks['tesseract'] and checks['tesseract_bilingual'])
    ))
    ...
    elif checks['tesseract'] and checks['tesseract_bilingual']:
        checks['ocr_provider'] = f'tesseract-{DEFAULT_TESSERACT_LANG}'
    else:
    ...
    checks['ocr_install_purpose'] = (
        'OCR 用于识别小红书图文笔记封面和全部内页图片里的中文和英文文字，提高分类和专辑归档准确性。'
    )
    if is_macos:
        ...
        checks['ocr_install_size'] = {
            'preferred_provider': f'tesseract-{DEFAULT_TESSERACT_LANG}',
    ...
    if not ocr_checked:
        ...
        if is_macos:
            ...
            checks['ocr_install_suggestions'] = [
                '安装 Tesseract OCR、chi_sim 简体中文和 eng 英文语言包，并把 tesseract.exe 加入 PATH。',
                '确认 `tesseract --list-langs` 同时包含 chi_sim 和 eng 后，才能判定中英文 OCR 就绪。',
            ]
        else:
            checks['ocr_install_suggestions'] = [
                '安装 Tesseract OCR、chi_sim 简体中文和 eng 英文语言包。',
            ]
        checks['ocr_install_required'] = True
        checks['ocr_install_authorized_by_enable_switch'] = True
        checks['should_ask_user_to_install_ocr'] = False
        checks['ocr_message'] = '用户已开启图文 OCR；当前缺少可用中英文 OCR，应按当前平台安装并复验。'
    else:
        checks['ocr_install_suggestions'] = []
        checks['ocr_install_required'] = False
        checks['ocr_install_authorized_by_enable_switch'] = True
        checks['should_ask_user_to_install_ocr'] = False
        checks['ocr_message'] = '已找到可复用的中英文 OCR，无需安装。'
