...
- 普通分类的 analysis provider 三选一：`codex-cli`、本地 `mimo-vl-mlx`、`command`/宿主 Agent 适配器。这个 Skill 不强制使用 Codex CLI；深度静态报告是例外，必须满足下文固定的 MiMo 听觉 + MiMo-VL 完整时轴 + 画面 OCR 证据合同。
...
- RED Skill 包只保留标准 `SKILL.md`、许可证和一次性 WorkBuddy 安装器。它明确披露权限、禁止自动运营账号，并在用户确认后从公开 GitHub 仓库安装完整 Plugin；不得把开发仓库、私有接口研究或 Plugin/MCP 构建产物直接上传小红书审核。
...
不要只上传 `SKILL.md`。两个渠道包都必须包含一次性 WorkBuddy 安装器；完整 Plugin 保留在公开 GitHub 仓库中。
...
脚本会在输出前校验顶层目录、frontmatter、渠道必需文件、RED Skill 权限声明、文件数、扩展名、单文件 10 MB 和总大小 30 MB 限制；任一条件不满足都会失败，不会生成可误传的“通过”结果。
...
| --- | --- | --- | --- |
| 快速整理 | 关闭 | 关闭 | 关闭 |
| 轻度整理 | 开启 | 平台字幕 / 轻量音频 | 关闭 |
| 深度整理 | 开启 | MiMo 听觉 | MiMo-VL + 画面 OCR |

快速整理跳过 OCR 和视频功能卡，不安装、不运行任何内容识别组件。轻度整理对图文执行完整图片 OCR；视频只使用可核验的平台字幕或轻量音频转写，不抽取或分析完整时轴画面，也不运行 MiMo-VL。深度整理对图文执行完整图片 OCR；每条视频必须同时通过 MiMo 听觉转写、MiMo-VL 完整时轴画面分析和逐帧画面 OCR 三层质量门，不能用 Codex CLI、通用宿主视觉或其他 provider 替代其中任何一层。选择档位不授权打开浏览器或移动笔记；若需要安装，会先显示实际缺失组件和体积，系统权限窗口仍由用户确认。

整理深度确定后、浏览器授权前，可选择是否同时生成静态 HTML 专辑报告；用户已经明确要求文件时不重复询问。三个档位都支持报告。用户选择生成报告或明确要求文件后，静态 HTML 是正式交付，对话文本只临时预览结论并提供文件入口；不生成报告时才只在当前对话展示简短结果。每个专辑会生成：

- 桌面入口 `小红书专辑《<专辑名称>》<模式>报告.html`，负责给出明确的专辑结论、主题地图、阅读顺序和可点击的逐条卡片；
- 同级目录 `小红书专辑《<专辑名称>》<模式>报告/`，每条笔记一个独立静态 HTML；`<模式>` 只能是“快速”“轻度”或“深度”。

用户同时要求轻度和深度时，会生成两套相互独立、可同时存在的 HTML 与目录，不复用路径、不互相覆盖；两套总览和逐条页都会醒目标出当前模式，且不生成跨扫描深度链接。

轻度报告不是标题导航：图文以完整图片 OCR 为证据；视频以通过质量门的平台字幕或轻量音频转写为证据，输出实际内容概括、关键观点和查看重点，同时明确没有核验完整时轴画面。两种文字证据都无法形成可用正文时，单条页会直接标为“轻度证据不足，建议生成深度报告”，不会用“当前文字稿不足以确认视频时间主题”“标题信息与文字稿内容无法互相印证”等空泛占位话，也不会从标题、封面或元数据猜正文。

深度视频报告必须同时使用通过质量门的 MiMo 听觉转写、MiMo-VL 完整时轴画面分析和逐帧画面 OCR；单条页固定包含“一句话总结 / 这条到底讲什么 / 直接结论 / 具体观点 / 可直接采用 / 注意边界”，并提供有时间戳的“值得补看”。任一必需证据层缺失、失败、哈希不一致或时轴覆盖不完整时，该条深度结论会硬停止并标为“深度证据生成失败”，不会退回轻度报告、音频单模态、标题、封面、元数据或其他视觉 provider，也不会声称整份深度报告已完整生成。报告只写本地文件，不包含小红书发布或任何账号写入功能。

轻度报告与深度报告彼此独立：总览页和单条页都不提供跨扫描深度跳转，单条页只返回当前深度的专辑总览。用户选择哪一种扫描深度，就只需要交付对应的一套报告。

报告生成统一使用 `scripts/generate_album_report.py`，不手写 HTML。深度报告先由 `scripts/run_album_mimo_vl_timelines.py` 读取宿主帧 manifest 与缓存，`scripts/assemble_album_visual_evidence.py` 严格绑定 MiMo 听觉、MiMo-VL 与逐帧 OCR，`scripts/build_album_deep_details.py` 生成视频逐条内容；随后把视频与图文 details 作为可重复的 `--detail-bundle` 传给 `scripts/finalize_album_deep_report_data.py`。finalizer 不限定专辑条数或图文/视频比例，只有 note id、证据 hash、完整时轴与完整图片 OCR 全部精确匹配时才输出可交给 HTML 生成器的最终数据。

例如，已有完整的轻度逐条内容时，可在仓库根目录执行：

```bash
python3 scripts/generate_album_report.py \
  --items visible_items.json \
  --classification classification.json \
  --board-name "<专辑名称>" \
  --depth light \
  --state verified \
  --membership-snapshot board_snapshot.json \
  --synthesis album_synthesis.json \
  --details light_details.json \
  --output-dir "$HOME/Desktop"
```

命令只写本地静态文件，不发布小红书，也不修改账号。

WorkBuddy Plugin 当前只支持快速和轻度整理，且没有视频语音与完整时轴画面能力；它不能生成或冒充上述深度视频报告。用户选择深度整理时，必须在打开 WorkBuddy 专用浏览器前停止；只有用户另行明确授权非 WorkBuddy 的具体浏览器后，才可改走直接路径。
