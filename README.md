...
> 当前版本：**v2.0.0**。更新记录见 [CHANGELOG.md](CHANGELOG.md)。
...
- v2.0 支持快速启动和完整启动：先选择模式、范围和 OCR，再使用当前授权浏览器读取真实列表；读取后才按真实图文/视频分布询问视频内容识别。
- 支持两种读取方式：被动手动模式一次只读当前已显示卡片；受控分段模式由用户一次设置每组 1–200 条和暂停分钟数，并独立保存每组进度。
...
- 同一次整理使用同一份 `xhs_safety_state.json`：下游脚本会自动继承输入文件旁已有的状态；若把工件放在不同目录，必须对每个小红书访问命令传同一个 `--safety-state <路径>`。检测到安全验证、异常访问、登录页、页面绑定丢失或写入状态不确定后会立即停机；用户完成平台处理并在当次重新确认浏览器后，才可建立新的活动会话引用已保存进度。
...
- 开启图文 OCR 时默认使用 Tesseract + 简体中文 `chi_sim` 与英文 `eng`；EasyOCR 只作为用户明确选择的 GB 级替代方案：
...
完整交互文本与执行边界见 [v2.0 新用户启动与整理流程](references/onboarding-v2.md)。实际顺序是：

1. 选择 **快速启动（按推荐设置）** 或 **完整启动（自己逐项设置）**；
2. 选择收藏、点赞或我全都要；
3. 两种模式都先问是否开启 OCR（推荐开启，以提高识别率），先检查已有能力，缺失才询问安装；
4. 用户确认本次使用的已登录浏览器、目标列表页和 JavaScript 能力；
5. 一次设置每组数量（最多 200）和暂停时间（默认 3 分钟），再开始读取；
6. 读取完成后才展示真实图文/视频分布，并按需询问视频声音和画面识别；
7. 先生成本地的**分类方案预览**，默认只显示每类数量，用户需要时才展开条目；
8. 单独确认后才创建专辑或移动笔记；第一次完整整理后可选择每周整理新内容。每周仍需要当次浏览器确认，且永远不会自动移动；`weekly_delta.py` 只有在新的分类方案覆盖全部新增条目后才更新已处理基线。

读取、分类和移动分别保存状态。出现平台验证或异常访问时，Skill 会保存进度并停止；用户完成验证、回到列表页并回复“继续整理”后，才检查当前状态并从未完成段继续。

用于保存 v2.0 设置的本地命令：

```bash
python3 scripts/onboarding_config.py run_config.json \
  --startup-mode quick --source collection --ocr enabled \
  --batch-size 200 --pause-minutes 3
```

## 旧版文档（已归档）

新用户和运行中的 Agent 只使用上方 v2.0 流程。以下旧版文字不参与运行，仅保留在源码历史中以便追溯。

<!-- v1-archived-onboarding-start
...
当前宿主 Agent 已明确知道自己具备视觉能力时，可加 `--host-visual-capability ready --host-visual-name "<名称>"`；无法证明时保持默认 `unknown`。预检只检查已配置路径和本地组件，不访问浏览器、不联网、不安装软件、不加载大型模型。它会分别报告中英文 OCR、本地语音模型、本地视觉模型和宿主视觉能力；检查结果不等于启用授权。
...
> **选择整理深度**
>
> 1. **快速整理**：不读图片文字、不听视频、不看视频画面；只按标题、正文、标签和作者整理。
> 2. **轻度整理**：只读取图文笔记封面和全部内页图片中的文字；不听视频，也不看视频画面。
> 3. **深度整理**：读取图文全部图片文字，并结合视频字幕或语音与完整时轴真实画面分类。
>
...
>
> 回复“开启”：使用预检找到的中英文 OCR；如果没有，则安装当前系统的推荐组件。安装前显示具体组件、预计下载量和磁盘占用；系统权限窗口仍需用户确认。
>
> 交互时只显示当前系统对应的一项，其他系统的安装说明不要显示：
>
> - **macOS：**优先复用系统 Vision，OCR 模型本身通常是 **0 MB 额外下载**；缺少 Swift/Command Line Tools 时需要 GB 级系统组件，实际大小以 macOS 安装窗口为准。
> - **Windows：**默认安装 Tesseract + `chi_sim` 与 `eng`，通常为**几十到数百 MB**，实际大小以当前安装器为准。建议默认位置是 `C:\Program Files\Tesseract-OCR\`，两个语言包通常位于其 `tessdata\` 子目录；用户可自行修改安装位置。
> - **仅当用户明确选择 EasyOCR 时：**通常是 **GB 级**。
>
> 回复“不开启”：不安装、不运行图文 OCR；预检结果不会被用于分类。仍可按标题、正文、标签和作者分类，但准确率可能下降。

回复“开启”只授权使用 OCR，并在缺失时安装推荐 OCR 组件，不授权浏览器、视频分析或移动笔记。已有中英文 OCR 就复用；缺失时按当前系统安装，再运行 `python3 scripts/check_environment.py --ocr` 复验。详细规则见 [图文 OCR 分类开关](references/image-ocr-classification.md)。
...
下面是 v2.0 的本地配置和无写入准备命令。浏览器相关命令只能在用户本次明确授权的、已经登录并打开目标列表页的浏览器上运行。

```bash
cd ~/.hermes/skills/social-media/xiaohongshu-web-collection-organizing

# 保存本次快速启动设置；这条命令不打开浏览器
python3 scripts/onboarding_config.py run_config.json \
  --startup-mode quick --source collection --ocr enabled \
  --batch-size 200 --pause-minutes 3

# OCR 已开启时做本机复验；仍不打开浏览器
python3 scripts/check_environment.py --ocr

# 当次已授权 Arc 的第一组读取。每组都会生成独立文件。
# 上层 Agent 必须在暂停时间满足后、且用户没有说“暂停”时，才决定是否调用下一段；
# 此脚本自己不会滚动、刷新、打开 URL 或进入下一段。
python3 scripts/extract_visible_items.py visible_items.json \
  --backend macos-arc --source collection --capture-mode segmented \
  --batch-size 200 --pause-minutes 3 --segment-index 1 \
  --auto-continue --user-authorized --manifest crawl_manifest.json \
  --arc-window-id '<window-id>' --arc-tab-id '<tab-id>' \
  --arc-tab-marker '<window-name-marker>' \
  --arc-expected-url-substring '<expected-path>'
```

读取完成后，先在本地依次完成 OCR、视频内容 memo（如已开启）和 `classification.json`，展示分类方案预览；只有再次确认后才可运行真实移动。

<!-- v1-archived-command-path-start
