...
'''
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch('xhs_ocr_common.download_image', side_effect=self.fake_valid_image_download), \
                ...
                swift_result = perform_ocr_for_items(
                    [item], output, provider='swift', tesseract_lang='chi_sim+eng',
                )[0]
            ...
            with patch('xhs_ocr_common.run_ocr', return_value=ocr_result('中英联合', 'tesseract')) as changed_language_ocr:
                bilingual_result = perform_ocr_for_items(
                    [item], output, provider='tesseract', tesseract_lang='chi_sim+eng+deu',
                )[0]
    ...
    def test_tesseract_default_language_matches_bilingual_environment_requirement(self):
        completed = subprocess.CompletedProcess(
            args=[], returncode=0, stdout='中文文字\n', stderr='',
        )
        with patch('xhs_ocr_common.shutil.which', return_value='/usr/bin/tesseract'), \
                patch('xhs_ocr_common.tesseract_language_ready', side_effect=lambda language: language in {'chi_sim', 'eng'}), \
        ...
        self.assertEqual(command[command.index('-l') + 1], 'chi_sim+eng')
        self.assertEqual(result['text'], '中文文字')

    def test_ocr_cli_rejects_tesseract_without_bilingual_language_setting(self):
        with tempfile.TemporaryDirectory() as tmp:
            src = Path(tmp) / 'image_items.json'
            src.write_text('[]', encoding='utf-8')
            result = subprocess.run(
                [
                    sys.executable, str(SCRIPTS / 'ocr_note_images.py'),
                    str(src), str(Path(tmp) / 'ocr_results.json'),
                    '--provider', 'tesseract', '--tesseract-lang', 'chi_sim',
                ],
                cwd=str(ROOT), text=True, capture_output=True,
            )
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('chi_sim', result.stderr)
        self.assertIn('eng', result.stderr)

    def test_macos_vision_script_verifies_and_configures_bilingual_languages(self):
        script = (SCRIPTS / 'ocr_image.swift').read_text(encoding='utf-8')
        self.assertIn('supportedRecognitionLanguages()', script)
