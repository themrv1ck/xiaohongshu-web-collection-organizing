...
from check_environment import tesseract_ocr_probe  # noqa: E402
...
'''
    ...
    def test_auto_ocr_provider_requires_working_vision_or_bilingual_tesseract(self):
        with (
    ...
    def test_tesseract_requires_both_chinese_and_english_language_data(self):
        with (
            patch('xhs_ocr_common.shutil.which', return_value='/usr/bin/tesseract'),
            patch('xhs_ocr_common.tesseract_language_ready', side_effect=lambda language: language == 'chi_sim'),
        ):
            with self.assertRaisesRegex(RuntimeError, 'eng'):
                run_tesseract_ocr(Path('/tmp/not-used.png'), languages='chi_sim+eng')

    def test_environment_probe_rejects_chinese_only_tesseract(self):
        with (
            patch('check_environment.check', side_effect=lambda command: command == 'tesseract'),
            patch('check_environment.tesseract_language_ready', side_effect=lambda language: language == 'chi_sim'),
            patch('check_environment.tesseract_languages_ready', return_value=False),
        ):
            probe = tesseract_ocr_probe()
        self.assertTrue(probe['found'])
        self.assertTrue(probe['chi_sim'])
        self.assertFalse(probe['eng'])
        self.assertFalse(probe['bilingual'])
