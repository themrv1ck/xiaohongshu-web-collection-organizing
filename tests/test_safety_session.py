...
from extract_visible_items import (  # noqa: E402
    SegmentedCapturePauseError,
    capture_current_segment_with_js,
    capture_segmented_segment_with_js,
    extract_playwright,
...
class SafetySessionTests(unittest.TestCase):
    ...
    def test_segmented_capture_requires_explicit_safe_configuration_before_page_access(self):
        calls = []
        ...
        invalid_cases = (
            ({"batch_size": 201}, "--batch-size"),
            ({"pause_minutes": 0}, "--pause-minutes"),
            ({"auto_continue": False}, "--auto-continue"),
            ({"user_authorized": False}, "--user-authorized"),
            ({"segment_index": 0}, "--segment-index"),
        )
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            for index, (override, message) in enumerate(invalid_cases, start=1):
                with self.subTest(message=message):
                    ...
                    with self.assertRaisesRegex(ValueError, message):
                        capture_segmented_segment_with_js(
                            js_eval,
                            root / f"segment-{index}-visible.json",
                            manifest=root / f"segment-{index}-manifest.json",
                            **config,
                        )

        self.assertEqual(calls, [])

    def test_segmented_capture_records_contract_and_enforces_pause_without_scroll(self):
        calls = []
        payload = self.capture_payload(count=205)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            out = root / "visible_items.segment-001.json"
            manifest = root / "crawl_manifest.segment-001.json"
            state = root / "xhs_safety_state.json"
            result = capture_segmented_segment_with_js(
                js_eval,
                out,
                batch_size=200,
                pause_minutes=5,
            ...
            rows = json.loads(out.read_text(encoding="utf-8"))
            saved_manifest = json.loads(manifest.read_text(encoding="utf-8"))
            saved_state = load_safety_state(state)
            with self.assertRaises(SegmentedCapturePauseError):
                capture_segmented_segment_with_js(
                    js_eval,
                    root / "visible_items.segment-002.json",
                    batch_size=200,
                    pause_minutes=5,
                    auto_continue=True,
                    user_authorized=True,
                    segment_index=2,
                    manifest=root / "crawl_manifest.segment-002.json",
                    safety_state=state,
                )

        self.assertEqual(len(calls), 1)
        self.assertEqual(len(rows), 200)
        self.assertEqual(result["capture_mode"], "segmented")
        self.assertEqual(result["stopped_reason"], "batch_size_reached")
        self.assertEqual(saved_manifest["batch_size"], 200)
        self.assertEqual(saved_manifest["pause_minutes"], 5.0)
        self.assertTrue(saved_manifest["auto_continue"])
        self.assertTrue(saved_manifest["user_authorized"])
        self.assertFalse(saved_manifest["auto_scroll"])
        self.assertFalse(saved_manifest["auto_navigation"])
        self.assertTrue(saved_manifest["controller_required"])
        self.assertFalse(saved_manifest["resume_allowed"])
        self.assertEqual(saved_state["segmented_capture"]["last_completed_segment_index"], 1)
        self.assertNotIn("in_flight_segment_index", saved_state["segmented_capture"])
        self.assertTrue(all("window.scrollTo" not in call and "window.scrollBy" not in call for call in calls))

    def test_segmented_capture_halts_on_unclassified_exception_and_cannot_resume(self):
        first_calls = []
        second_calls = []
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(SafetyHaltedError):
                capture_segmented_segment_with_js(
                    broken,
                    root / "visible_items.segment-001.json",
                    batch_size=200,
                    pause_minutes=1,
                    auto_continue=True,
                    user_authorized=True,
            ...
            saved_state = load_safety_state(state)
            saved_manifest = json.loads((root / "crawl_manifest.segment-001.json").read_text(encoding="utf-8"))

        self.assertEqual(len(first_calls), 1)
        self.assertEqual(second_calls, [])
        self.assertEqual(saved_state["state"], "security_halted")
        self.assertEqual(saved_state["halt"]["reason_code"], "segmented_capture_exception")
        self.assertEqual(saved_manifest["segment_status"], "security_halted")
        self.assertEqual(saved_manifest["batch_size"], 200)
        self.assertTrue(saved_manifest["auto_continue"])

    def test_segmented_artifact_paths_are_independent_per_segment(self):
        base = Path("/tmp/visible_items.json")
        manifest = Path("/tmp/crawl_manifest.json")
        self.assertEqual(segment_artifact_path(base, 1).name, "visible_items.segment-001.json")
        self.assertEqual(segment_artifact_path(manifest, 12).name, "crawl_manifest.segment-012.json")
