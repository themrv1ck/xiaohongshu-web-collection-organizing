...
2. 先调用 `xhs_workbuddy_status`，并确认返回 `plugin_version=2.0.4`；缺失或版本不同都按第 11 步走官方 Plugin 安装/更新入口，重开 WorkBuddy 后再继续。版本正确但 `install_required=true` 时，先向用户说明需要把 Python Playwright 与 Chromium 下载到插件持久化目录并取得明确同意，再调用 `xhs_workbuddy_setup(install_dependencies=true)`，随后再次调用 status。
...
5. 登录工具返回后，立即把其 `target_page_url` 原样传给 `xhs_workbuddy_capture(browser_authorized=true,organizing_depth=quick|light)`，不得再次询问或让用户粘贴地址。WorkBuddy 插件必须在同一个专用 Chromium 会话中自动翻页：固定每 200 条独立保存一组，非最后一组真实等待 3 分钟后继续。轻度整理必须在关闭这次浏览器前，用只存在于插件进程内的原始卡片链接逐条打开全部已授权详情，用权威 `noteData.type` 纠正列表类型并取得完整图片列表；随后必须用同一个登录态 BrowserContext 下载图片字节到权限为 0600 的运行目录，只把相对本地路径与内容 SHA256 写入 `image_items.json`，清除源图片 URL 后再关闭浏览器并运行本地 OCR。Cookie、卡片原始 query、签名图片 URL 和 xsec 不得落盘、返回模型或出现在错误中。页面到底后只能在前端声明总数每次连读均未变化、真实唯一条数完全相等、且 `page_index ↔ note_id` 双向唯一并精确覆盖 `0..总数-1` 时结束；任一条缺少索引、同一笔记占多个位置、同一位置换笔记、总数变化/缺失、数量不符或索引缺口，都要保存已读数据并硬停，禁止把首屏约 10 条称为完整范围。列表采集不得点击、刷新、自动重试或写账号。只有 `ready_for_classification=true` 时插件才签发 capture receipt；WorkBuddy 必须自动传递，不得让用户查看、复制或保存。
6. 抓取完成后，先调用一次不带 `classification` 的 `xhs_workbuddy_prepare(...)`，并自动传入 capture 返回的 `evidence_receipt`。这一次只读生成与本次账号、页面和 `verify_pages` 绑定的 `board_snapshot.json`，返回 `phase=board_inventory`、`existing_board_names`、本次 `verify_pages` 和新的 inventory receipt；不得在拿到这份真实清单前猜测专辑。此阶段的 `classification_required` 是继续分类的阶段标记，不是失败或最终 blocker，必须立即继续第 7 步；若返回 `no_existing_boards`，才说明当前没有可用目标专辑并停止，不得生成预设类别。
7. 只能使用第一次 prepare 返回的完整脱敏 `classification_inputs`，并从 `existing_board_names` 中为全部真实 note id 逐条选择目标。该字段只含分类所需元数据、权威类型和已验证 OCR 文字，不含 href、图片 URL、下载路径或凭据；禁止让模型自行读取 `visible_items.json`、`image_items.json`、`ocr_results.json` 或运行目录。禁止创造新专辑或注入任何预设类别；没有准确匹配时保持 `target_board=""` 并进入人工复核。轻度整理必须先确认 capture 返回 `ready_for_classification=true`；禁止在 WorkBuddy 中运行无登录态 `enrich_note_images.py`，禁止 OCR 失败后静默改用元数据分类。
8. 把完整逐条分类和 inventory `evidence_receipt` 自动传给第二次 `xhs_workbuddy_prepare(classification=[...])`。工具必须先核验 MCP 内存账本与当前文件哈希，再核验 `crawl_manifest.json` 与本次完整抓取、账号、页面和整理档位一致；轻度整理还要机械核验 `image_items.json` / `ocr_results.json` 的完整 ID 顺序、权威前端来源、图片集合哈希、逐图状态和 OCR 运行指纹，再把合格 OCR 字段写入 `classification.json`。任一证据缺失或变化都必须在打开浏览器前停止，禁止改用元数据。随后核验分类 ID 与本次抓取完全一致、目标只属于刚才的真实专辑清单，并生成本次 `board_taxonomy.json`、`created_boards.json` 和硬闸门 dry-run；第二次调用复用已绑定快照，不再打开浏览器。只有返回 `phase=dry_run`、`mode=dry_run`、`ready_for_execute=true`、`blockers=[]`、`planned_move_count>0`、非空 `approval_digest` 和 plan receipt，才可请求执行确认。若 `planned_move_count=0`，禁止请求执行。
...
10. 用户明确确认逐条映射与本次移动上限后，原样传回绑定该上限和 `verify_pages` 的 `approval_digest`，由 WorkBuddy 自动带上 plan `evidence_receipt` 及 prepare 返回的同一 `verify_pages`，并调用 `xhs_workbuddy_execute(browser_authorized=true,user_confirmed=true,...)`。工具不再接收模型提供的 `expected_url_substring`，而是从已绑定 `page_url` 机械生成精确页面约束。`approval_digest` 是用户可见的方案编号，不能单独授权执行。Python 必须把最终分类、快照、目标专辑与报告按 receipt 哈希读入内存，启动本轮专用 Chromium，并只读核验精确 profile 路径、`tab` 与前端“我”账号；全部通过后才向 MCP 发出 `READY`。MCP 在 `COMMIT` 前再次重算所有文件哈希，单次消费 plan receipt 并回传 `COMMIT`；Python 收到后才允许第一次写入，且写入只使用已验证的内存快照，不重新按可变路径读取分类。`READY` 前的参数、文件、进程启动或页面绑定失败必须解除 receipt 占用、允许修正后重试。MCP/WorkBuddy 重启后旧 receipt 必须失效，旧目录只能查看；要继续写入必须重新 capture 和两阶段 prepare，不得根据本地 manifest 恢复信任。应用层 receipt 防止误调用、重复执行和证据漂移，但不把同一系统用户的恶意代码当作已隔离；正式使用依赖 WorkBuddy 的 OS 沙箱和最小终端权限。
...
WorkBuddy 路径由插件显式注入 `XHS_HOST=workbuddy`；插件浏览器入口会再次在代码层强制为 Playwright 自带 `chromium`、可见窗口和插件独立 profile，并拒绝 CDP、headless 与其他 `user-data-dir`。这条约束与选择 GLM、Hy3 或其他模型无关。完整安装和工具合同见 `references/workbuddy-plugin.md`。
