...
2. 先调用 `xhs_workbuddy_status`，并确认返回 `plugin_version=2.2.0`；缺失或版本不同都按第 11 步走官方 Plugin 安装/更新入口，重开 WorkBuddy 后再继续。版本正确但 `install_required=true` 时，先取得一次依赖安装同意，再调用 `xhs_workbuddy_setup(install_dependencies=true)` 并复验 status。Windows 只安装 Python Playwright 并检查系统 Edge，不下载 Chromium；macOS/Linux 安装插件独立 Chromium。
...
9. 普通整理结果直接在当前对话里用简短纯文本报告，不调用可视化 Skill、可视化指南、组件渲染、仪表盘或 `present_files`，也不为已有 JSON 产物额外生成展示文件。唯一固定例外是第 3 步已记录 `report_requested=true`：执行与最终只读成员核验全部成功后，必须运行 `scripts/generate_collection_report.py --board-snapshot <最终完整快照> --classification <同批完整分类> --output "$HOME/Desktop/我的小红书专辑整理报告.html"`。生成器按专辑列出主题、内容类型、已保存主题和全部笔记摘要；它只接受成员与分类逐条一致的完整快照。缺页、重复、数量变化或目标不一致时直接报错停止，不得重读笔记、补分析或猜写内容。除此之外，只有用户明确要求图表、网页或文件交付时才允许生成展示文件。普通文本报告仍只列工具顺序、抓取/分类数量、`mode`、`ready_for_execute`、`blockers`、`warnings`、`planned_move_count`、是否写入账号、`run_dir`，以及已正确归档/待复核条目。
...
11. 第 10 步完整核验通过后，若 `report_requested=true`，运行 `scripts/generate_collection_report.py --board-snapshot <post snapshot> --classification <同批完整 classification.json> --output "$HOME/Desktop/我的小红书专辑整理报告.html"`。报告按专辑说明主题、内容类型、已保存主题和主要笔记内容。生成器必须验证快照成员与分类 ID/目标逐条一致；任一缺页、重复、数量变化或分类目标不一致都停止。若 `report_requested=false`，不得生成 HTML。报告阶段不打开浏览器、不重新 OCR/转写/分析，也不执行任何小红书写入。
