...
当用户要求整理“小红书收藏 / 点赞 / 我的收藏 / 专辑分类”且本轮尚未明确范围时，必须先按下方 **v2.0 新用户入口** 选择“快速启动”或“完整启动”，再询问“收藏”“点赞”或“我全都要”。用户回答“收藏”就只整理收藏里的笔记；回答“点赞”就只整理点赞里的笔记；回答“我全都要 / 全部 / 都要”就把点赞和收藏合并去重后一起整理。读取、分类和移动是三件不同的事：先按用户选择分段读取并落盘，再在本地生成分类方案，最后单独取得创建专辑或移动笔记的确认。
...
本节优先级高于本文件下方全部旧版说明。执行前必须阅读 [v2.0 新用户启动与整理流程](references/onboarding-v2.md)；若文字冲突，以该文件和本节为准。不要把旧版“快速整理 / 轻度整理 / 深度整理 / 自定义”当作 v2.0 的新用户选项。

1. 先显示欢迎页，不要先问范围，也不要先检查浏览器：
...
2. 用户选择启动方式后，才询问范围：收藏、点赞、我全都要。范围确认后，两种模式都先询问 OCR，而不是先读取列表或先要求安装视频模型：
   ...
   - 用户选择开启后，先运行 `python3 scripts/check_environment.py --capability-preflight` 和 `python3 scripts/check_environment.py --ocr`；已有中英文 OCR 直接复用。缺失时才显示当前系统的组件、实际体积与安装确认。macOS 不显示 Windows 路径；Windows 可以显示 Tesseract 的默认位置 `C:\Program Files\Tesseract-OCR\`。用户不开启时分类必须显式传 `--skip-ocr`。
   - OCR 的“推荐安装”只表示它是轻量、能覆盖大量图文场景的能力；视频声音和画面能力是按需选择，不能被 OCR 同意隐式安装。

3. OCR 选择后，取得**本次**对一个已打开、已登录、正处于目标列表页面的浏览器的明确授权。提醒用户页面需要允许必要 JavaScript；只检查该页面、登录状态和 JavaScript 能力，不能在未确认前创建专辑或移动笔记。不得自动切换到其它浏览器，也不得复用旧会话的浏览器许可。

4. 读取前只问一次批次设置：默认“每组最多 200 条，暂停 3 分钟”；用户可以改为“每组 `<1–200>` 条，暂停 `<正整数>` 分钟”。把设置写入 `run_config.json`：
   ...
   用户本次确认后，已获授权的任务执行器可以在用户没有说“暂停”时按该节奏安排下一组；每组独立落盘。`--capture-mode segmented` 本身只校验、读取并保存当前组，不会自行滚动、刷新、打开 URL 或续段。被动手动模式仍可用，且必须保持不滚动、不自动续段。暂停是用户工作节奏，不是规避验证或伪装真人的手段。首次实际读取后才显示 `<已读取>/<页面声明总数>`；没有可靠总数时只能显示已读取数量，不能预估或编造。

5. 全部所选列表读取完毕后，显示真实图文、视频、待确认分布。快速启动根据分布给出推荐；完整启动逐项设置分类规则、视频声音、视频画面和已有专辑策略。视频能力只能在此时询问，并且要给例子：声音/字幕可以把“太香了”的番茄炒蛋教程归入“做饭 / 菜谱”；无声的衣柜整理视频需要真实画面才能可靠归到“收纳整理”。

   - 快速启动的视频选项是“耳朵和眼睛都要”“先检查我是否已有类似功能的 AI 或模型”“不开启”；完整启动才可逐项选择声音和画面，因此可额外提供“只分析声音”。不得把视觉分析固定为 Codex CLI；可复用已有宿主 Agent/API、`mimo-vl-mlx` 或用户明确的其他视觉能力。
   - 缺少对应模型时先显示用途、大小和安装确认：`MiMo-V2.5-ASR-MLX` 约 6.6 GB，`MiMo-VL-7B-RL-2508` 约 16.6 GB。MiMo-VL 只看画面，声音来自字幕或 ASR。

6. 分类必须先在本地生成 `classification.json`，然后显示**分类方案预览**：建议专辑名、每类数量、低置信度与待确认数量。默认不展开每个专辑里的笔记，必须再问用户是否展开审阅。没有用户确认前，不创建专辑、不移动任何笔记。

7. 发生安全验证、异常访问、登录提示、页面绑定丢失或状态不确定时，立即落盘并停止。告诉用户完成平台处理、回到列表页后回复“继续整理”。收到后只检查当前页面与保存进度是否一致，并从未完成段恢复；不得重读已保存数据、不得绕过 `security_halted`、不得自动重试或继续移动。

8. 第一次完整整理完成后，询问是否开启每周整理。启用后复用范围、批次、OCR、视频、分类和审阅配置，只整理新内容并先生成新方案；不能自动创建专辑或移动笔记。每周触发时仍须取得当次浏览器确认，不能用一周前的浏览器许可。每周读取后先运行 `scripts/weekly_delta.py` 得到新增笔记预览；只有新的 `classification.json` 覆盖全部新增 note id 后，才允许提交 `weekly_processed_note_ids.json` 基线。

## 已废弃的 v1 兼容说明（不得用于 v2.0 新用户）

以下内容只为历史脚本字段追溯而保留，不是用户交互或执行入口；新运行必须遵从上方 v2.0 入口和 `references/onboarding-v2.md`。

<!-- v1-archived-workflow-start
0. 首次运行顺序固定为“整理范围 → 本地能力只读预检 → 快速启动档位 →（仅自定义时）图文 OCR / 视频内容分类”。只读预检在范围确认后自动执行，不需要单独开关；它不访问浏览器、不联网、不安装软件、不加载大模型。快速启动档位会直接确定两个功能开关；只有用户回复“自定义”时，才逐项询问 OCR 和视频。
   ...
   - 显示预检结果后，先显示快速启动卡片：
     ...
     >
     > 1. **快速整理**：不开启图文 OCR、视频语音识别或视频画面识别；只按标题、正文、标签和作者整理。
     > 2. **轻度整理**：只开启图文 OCR；读取图文笔记封面和全部内页图片中的文字，不听视频，也不看视频画面。
     > 3. **深度整理**：开启图文 OCR、视频语音识别和视频画面识别；图文读取全部图片文字，视频结合字幕或语音与完整时轴真实画面分类。
     >
   ...
   - 只有用户回复“自定义”时，再显示图文 OCR 卡片：
     ...
     >
     > 回复“开启”：使用刚才检查到的中英文 OCR；如果没有，则安装当前系统的推荐组件。安装前显示具体组件、预计下载量和磁盘占用；系统权限窗口仍需用户确认。
     >
     > 请只显示当前系统对应的一项，其他系统的安装说明不要显示：
     >
     > - **macOS：**优先复用系统 Vision，OCR 模型本身通常是 **0 MB 额外下载**；缺少 Swift/Command Line Tools 时需要 GB 级系统组件，实际大小以 macOS 安装窗口为准。
     > - **Windows：**默认安装 Tesseract + 简体中文 `chi_sim` 与英文 `eng`，通常为**几十到数百 MB**，实际大小以当前安装器为准。建议默认位置是 `C:\Program Files\Tesseract-OCR\`，两个语言包通常位于其 `tessdata\` 子目录；用户可自行修改安装位置。
     > - **仅当用户明确选择 EasyOCR 时：**通常是 **GB 级**，不自动作为默认方案。
     >
  ...
  - 运行时以预检的系统类型为准：macOS 用户的 OCR 提示不得出现 Windows 的 `Tesseract-OCR` 安装路径、`tessdata` 或 Windows 命令；Windows 用户也不得看到 macOS 的 Command Line Tools 说明。
...
1. 确定快速启动档位，或在自定义路径完成两个功能开关后，再做所选路径的完整环境复验：操作系统、明确授权的浏览器网页登录态、浏览器自动化后端，以及用户已开启的能力。首次预检不能替代这里的完整验证。
   ...
   - **只有图文 OCR 开关已开启时**，运行 `python3 scripts/check_environment.py --ocr`（Windows：`python scripts/check_environment.py --ocr`），读取 `ocr_checked` / `ocr_status` / `ocr_ready` / `ocr_provider` / `tesseract_chi_sim` / `tesseract_eng` / `tesseract_bilingual` / `ocr_install_size`。若缺失，按 `references/image-ocr-classification.md` 安装并复验；不得重复询问安装同意，但系统权限窗口仍由用户确认。
   - 图文 OCR 开关未开启时，不得运行 `--ocr`、不得安装 OCR、不得执行 `enrich_note_images.py` 或 `ocr_note_images.py`。PaddleOCR 不是受支持 provider；Tesseract 缺少 `chi_sim` 或 `eng` 任一语言包时不得标为可用，也不得静默退回单语 OCR；EasyOCR 也不得自动作为默认替代。
